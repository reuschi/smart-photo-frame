...
**Computing Hardware:** Every RaspberryPi (or other Linux driven system) with wireless or wired connection and access to internet (successfully tested and running on ZeroW)\
...
allowedsenders = 123456789,<comma seperated Telegram user id's> ; will be allowed to send new pictures
...
If you want to send pictures via Telegram, you need to create your own Telegram Bot. For that, just follow the tasks:
...
For my use case I soldered them directly on the RasPi and gave the buttons that raise and lower the timer a higher hardware button to identify them without having a look on the board.
